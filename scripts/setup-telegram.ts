...
async function main() {
  const required = ["BOT_TOKEN", "APP_URL", "MINI_APP_URL", "TELEGRAM_WEBHOOK_SECRET"] as const;
  ...
  await api.setWebhook(`${appUrl}/api/telegram`, { secret_token: telegramWebhookSecret()! });
