...
type Copy = {
  ...
  montage: {
    colors: string[]; previousColors: string; nextColors: string; video: string; requirements: string; upload: string; uploadHint: string; stylesSoon: string;
    ...
    fileInvalid: string; videoInvalid: string; checking: string; ready: string; videoUnreadable: string; chooseVideo: string;
  ...
  videos: { title: string; loading: string; empty: string; failed: string; retry: string; download: string; trial: string; ready: string; expires: (date: string) => string; };
...
const ru: Copy = {
  ...
  montage: {
    colors: ["Янтарь", "Лазурь", "Лайм", "Багровый", "Жемчуг", "Бирюза", "Фиолет", "Розовый неон", "Белый", "Оранжевый"], previousColors: "Предыдущие цвета", nextColors: "Следующие цвета", video: "Видео", requirements: "Требования к видео", upload: "Загрузите видео", uploadHint: ".mp4, .mov, .mkv, .webm · 2–30 сек · до 100 МБ", stylesSoon: "СКОРО", parameters: "Параметры", generate: "Создать ролик", trial: "Сделать пробный ролик", topUp: "Пополнить баланс", connecting: "Подключаем Telegram…", processing: "Монтируем ролик…", trialHint: "с водяным знаком", topUpHint: (missing) => `не хватает ${missing} токенов`, balance: "БАЛАНС", tokens: "токенов", fileInvalid: "Нужен MP4, MOV, MKV или WebM до 100 МБ.", videoInvalid: "Нужно видео длительностью от 2 до 30 секунд.", checking: "Проверяем видео…", ready: "Ролик готов к загрузке.", videoUnreadable: "Не удалось прочитать параметры видео.", chooseVideo: "Сначала выберите видео.", projectFailed: "ИИ-сервис не смог обработать ролик. Если токены списывались, они возвращены.", projectReady: "Ролик готов — ссылка отправлена в чат с ботом.", uploadFailed: "Загрузка не удалась.", createFailed: "Не удалось создать проект.", queueFailed: "Не удалось запустить монтаж.", trialQueued: "Пробный ролик поставлен в очередь. Он будет доступен с водяными знаками.", queued: "Монтаж поставлен в очередь. Это может занять несколько минут.", introClose: "Понятно", introTitle: "Бот для автомонтажа", introSubtitle: "Загрузите свой ролик",
  },
  requirements: { title: "Требования к видео", carousel: "Примеры видео", correct: "Показать подходящий пример", incorrect: "Показать неподходящий пример", paragraphs: ["Загрузите любое видео длиной от 2 до 30 секунд. Поддерживаются MP4, MOV, MKV и WebM, размер файла — до 100 МБ.", "Лучше всего подойдут ролики с понятной речью и главным героем в кадре.", "ИИ превратит исходник в динамичный Instagram/Reels-монтаж с современными субтитрами выбранного цвета."] },
  ...
  support: { title: "Поддержка", subtitle: "Короткие ответы на частые вопросы. Если нужного нет — напишите оператору, разберёмся.", contact: "Написать в поддержку", questions: [
    ...
    { question: "В каком формате будет готовый ролик?", answer: "Обработка подходит для вертикальных, горизонтальных и квадратных исходников. ИИ адаптирует композицию под динамичный Instagram/Reels-стиль, добавляет современные субтитры и сохраняет важное содержание видео." },
  ...
  videos: { title: "Ролики", loading: "Загружаем ролики…", empty: "Готовых роликов пока нет.", failed: "Не удалось загрузить ролики.", retry: "Повторить", download: "Скачать", trial: "Пробный ролик · с водяным знаком", ready: "Готов к скачиванию", expires: (date) => `хранится до ${date}` },
...
const en: Copy = {
  ...
  montage: {
    colors: ["Amber", "Azure", "Lime", "Crimson", "Pearl", "Turquoise", "Violet", "Neon pink", "White", "Orange"], previousColors: "Previous colors", nextColors: "Next colors", video: "Video", requirements: "Video requirements", upload: "Upload video", uploadHint: ".mp4, .mov, .mkv, .webm · 2–30 sec · up to 100 MB", stylesSoon: "SOON", parameters: "Settings", generate: "Create video", trial: "Create trial video", topUp: "Top up balance", connecting: "Connecting Telegram…", processing: "Editing video…", trialHint: "with watermark", topUpHint: (missing) => `${missing} tokens needed`, balance: "BALANCE", tokens: "tokens", fileInvalid: "Use MP4, MOV, MKV, or WebM up to 100 MB.", videoInvalid: "Use a video from 2 to 30 seconds.", checking: "Checking video…", ready: "Video is ready to upload.", videoUnreadable: "Could not read the video details.", chooseVideo: "Choose a video first.", projectFailed: "The AI service could not process the video. Any charged tokens were returned.", projectReady: "Your video is ready — a link was sent in the bot chat.", uploadFailed: "Upload failed.", createFailed: "Could not create the project.", queueFailed: "Could not start editing.", trialQueued: "Trial video is queued. It will include watermarks.", queued: "Editing is queued. It may take a few minutes.", introClose: "Got it", introTitle: "Automatic video editing bot", introSubtitle: "Upload your video",
  },
  requirements: { title: "Video requirements", carousel: "Video examples", correct: "Show a suitable example", incorrect: "Show an unsuitable example", paragraphs: ["Upload any video from 2 to 30 seconds. MP4, MOV, MKV, and WebM are supported up to 100 MB.", "Videos with clear speech and a main person in frame work best.", "The AI turns the source into a dynamic Instagram/Reels edit with modern subtitles in your selected color."] },
  ...
  support: { title: "Support", subtitle: "Quick answers to common questions. If yours is missing, write to our support team.", contact: "Contact support", questions: [
    ...
    { question: "What format will the finished video use?", answer: "Portrait, landscape and square sources are supported. The AI adapts the composition into a dynamic Instagram/Reels style, adds modern subtitles and preserves the important content." },
  ...
  videos: { title: "Videos", loading: "Loading videos…", empty: "No completed videos yet.", failed: "Could not load videos.", retry: "Try again", download: "Download", trial: "Trial video · with watermark", ready: "Ready to download", expires: (date) => `stored until ${date}` },
