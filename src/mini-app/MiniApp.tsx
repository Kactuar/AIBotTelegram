...
export function Icon({ name }: { name: "moon" | "coins" | "play" | "upload" | "spark" | "people" | "films" | "profile" | "help" | "support" | "channel" }) {
...
export default function MiniApp() {
  ...
  const header = <header className={styles.header}><strong>Brandly<span>.</span></strong><div><button aria-label={t.common.darkTheme} className={styles.roundButton} onClick={() => setDark(!dark)}><Icon name="moon" /></button><button className={styles.balance}><Icon name="coins" />{authorized ? balance : "—"}</button></div></header>;
  ...
  if (tab === "invite") return <section className={`${styles.app} ${dark ? styles.dark : ""}`}><InviteScreen language={language} authorized={authorized} authPending={authPending} header={header} onBalance={setBalance} nav={<Nav language={language} active="invite" select={setTab} balance={authorized ? balance : undefined} />} /></section>;
